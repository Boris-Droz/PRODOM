...
####################################################################################################################################
####################################################################################################################################
## Historic (major update step)
## --------
## v7.0 September 2016 - Boris DROZ & Gerrad Jones, ETHZ & EAWAG
## v8.0 Januar 2017 - Boris DROZ --> more ML technic implemented
## v13 August 2022 adapt for non spatial data + update for R4.2.0 and new package version
## v14 October 2022 - support vector classification option
## v14.6 April 2023 --Error message is provide to help the user.
...
f.dat <- "PRODOM_spectra.DBPv1.csv"# File name of data points
...
## Ratio occurrence -- 
## threshold ratio of occurrence necessary for the quantitative model
# if proportion is lower or equal to RA.0
## quantitative model is not made but classification could be made.
RA.0 <- 0.5

# Similar for the SVM classification model -- Ratio of absence (zero) values
## between where a classification is considered
# if proportion is lower or equal than the lower range 
## classification model stop but quantitative could be made.
# if proportion is higher or equal than the upper range classification model stop 
## but quantitative could not be made.
RA.nodat <- c(0.1,0.9)

# suport vector classification for qualitative classification
SVC <- "YES"
...
cat(paste("Which proportion of occurance you consider for quantitative ML model ?", RA.0),file= f.info,append=TRUE, sep="\n")
cat(paste("Which proportion (range) of zero you consider for runing support vector classification ?", RA.nodat),file= f.info,append=TRUE, sep="\n")
...
################################################################################
## set data in -....
names(data.in) <- unlist(lapply(strsplit(names(data.in),"_"),"[[",1))# removed unit in name
 
data.in <-cbind(data.in, per.comp.3= data.in$comp.3/   # add proportion comp3
                apply(data.in[,78:83],1, sum, na.rm=TRUE))

## had mold THM4 and HAA5
data.in <-cbind(data.in, n.THM4= data.in$TCM/119.38 + data.in$BDCM/163.8 +
              data.in$DBCM/208.28 + data.in$TBM/252.73)
data.in <-cbind(data.in, n.HAA5= data.in$DCAA/128.94 + data.in$TCAA/163.38)
...
for (tu in 1:length(var.y))# selected variable y to pred
{
  ...
  if (length(var.sele.0)==1)
  {
    c.p. <- var.list.0[var.list.0$var.name==var.y[tu],  names(var.list.0) == var.sele.0]
    names(c.p.) <- names(var.list.0) [names(var.list.0) == var.sele.0]
  ...
  var.list <- var.list.0# reload origin list
  ...
  if (SVC=="YES"){
    ...
    for (i in 1:n.rep)
    {
      ...
      # if within the range of Ra.nodata
      if (  1- sum(if.dat)/ length(c(y.vali,y)) >=RA.nodat[2] |
            1- sum(if.dat)/ length(c(y.vali,y)) <=RA.nodat[1] ) 
          {
          scv.cali[[i]] <- rep(1,length(y))
          scv.vali[[i]] <- rep(1,length(y.vali))
        
          if (i==1){
            
            f.info <- paste(outpath,"/README_No_model.txt",sep="")
            
            cat( paste("*** TWO OPTIONS.... ****"), 
                      file= f.info, append=TRUE,sep="\n")
            cat( paste("1) NO enough absence data or less than RA.nodata parameter set.
                       So no need to run classification model"),
                       file= f.info, append=TRUE,sep="\n")
            cat( paste("2) NO enough occurance or less than RA.nodata parameter set.
                       So no option to run classification model"),
                       file= f.info, append=TRUE,sep="\n")
            cat( paste("USER OPTION, is to play a little bit with RA.nodata parameter"),
                        file= f.info, append=TRUE, sep="\n")
  ...
  ###########################################
  if ( ( 1- sum(if.dat)/ length(c(y.vali,y)) )>=RA.0 )
  {
    f.info <- paste(output,"/README_No_model.txt",sep="")
    
    cat( paste("*** Not enough occurrence (detection) data to run a quantitative 
               model for this species under the parameter selected"), 
               file= f.info, append=TRUE, sep="\n")
    cat( paste("A potential option is to decrease the threshold parameter <RA.0> 
               and see what you get") , file= f.info, append=TRUE, sep="\n")
...
} # end loop too much zero in data (RA.0 parameter)
