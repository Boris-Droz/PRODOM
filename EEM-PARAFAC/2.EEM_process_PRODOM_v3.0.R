...
##  indices calculation with staRdom Package 
####
#####============================================================
...
# v2.1. --> Adjust Ramand-Quinine unit convertion , ABS 280 nm in addition
...
## Descriptif
##############
## 1) read sp file (from PE fluorometer) and convert it into EEM 
## 2) and performed EEM correction  following data processing prior to parafac analysis:
# Blank subtraction
...
# - /input/EEM creat into the 1.EEM_creatfromsp_PRODOM_v3.0.R script
# - /input/ABS

# self-creat an input folder with EEM and output folder with "EEM-corr" spectra.
...
## Lawaetz, A. J.; et al. Appl. Spectrosc. 2009, 63 (8), 936–940. https://doi.org/10.1366/000370209788964548.
...
{ 
     ...
     creat.subDir (paste(workdir,"/output",sep=""),list.folder[k])
    creat.subDir (paste(workdir,"/output/",list.folder[k],sep=""),"EEM_corr")
    creat.subDir (paste(workdir,"/output/",list.folder[k],sep=""),"plot_EEM_corr")
    ...
    for (n in 1:length(eem_list))
        {
          ...
          write.table(Sample_1, file=paste(workdir,"/output/",list.folder[k],"/EEM_corr/",eem_list[[n]]$sample,".csv" ,sep=""),
                                   sep = ",", quote = FALSE, row.names = FALSE, col.names = FALSE)
        }
    # save the plot one by one in a date folder
    #creat.subDir (paste(workdir,"/output/plot_EEM_corr",sep=""),list.folder[k])
    
    png(filename = paste(workdir,"/output/", list.folder[k], "/plot_EEM_corr/Rplot%03d.png",
    ...
    slope_parms <- abs_parms(abs_data, cuvle = 1, 
                             unit= c("absorption"), add_as = 280)
    ...
    for (q in 1:length(eem4peaks))
        {
          # quinine sulfate 
         qs.eem <-c(qs.eem, max(pracma::interp2(eem4peaks[[q]][["ex"]], eem4peaks[[q]][["em"]], 
                                                       eem4peaks[[q]][["x"]], 350,450))  ) 
      
          eem4peaks[[q]][["x"]] <- eem4peaks[[q]][["x"]]/RtoQ ## convert data in QSU
          ## index RtoQ is instrument specific see detail in
          ...
          hlf.eem_qsu <-c(hlf.eem_qsu, max(pracma::interp2(eem4peaks[[q]][["ex"]], eem4peaks[[q]][["em"]], 
                                            eem4peaks[[q]][["x"]], rep(280, length(425:475)), 425:475)) )
        }
    
    d1.out <- cbind(indices_peaks,slope_parms[,2:ncol(slope_parms)], 
                    tlf.eem_qsu=tlf.eem_qsu, hlf.eem_qsu=hlf.eem_qsu, QS.eem= qs.eem )
    
    d1.out <- cbind(tlf.eem_qsu=tlf.eem_qsu, hlf.eem_qsu=hlf.eem_qsu, QS.eem= qs.eem )
    
    #d1.out <-  cbind(indices_peaks, QS.eem= qs.eem )
    
    # data table of run loop values
    write.table(d1.out, file=paste(workdir,"/output/",list.folder[k] ,"/fluo_indice.txt",sep=""),
                sep="\t", append=FALSE, row.names=TRUE,col.names=TRUE, quote=FALSE) 
    
    d.out <- rbind(d.out,d1.out )
} # end list folder  
