...
# Set folder 
## =========
workdir.model <- "C:/Users/bodro/Documents/Actual/EPA 2019-W-MS43 - PRODOM/Data_analysis/ML_pred"
model.folder <- "/output/2023-08-09_mod.proj_Vlux"
...
f.dat <- "input/VLux_data_20211001-20220908_clean.csv"# File name of data points

## choose the name of the variable selection you will use 
## -- var.list column name with 
## 0 for not use and 1 for use
var.sele <- c("ML_Vlux") # one option at the time

TLF.range <- c(4.419,81.962) #range of calibration model
HLF.range <- c(4.855, 163.477)
...
## set model path
inpath <- paste(workdir.model,model.folder,sep="")
...
##################
data.in <- data.0
# rename to match var name with ML-model
names(data.in) <-c("TIME","CHLORO","tlf.eem","hlf.eem","abs","turb")

# restrict to range outside cali data
#data.in <- data.in[data.in$tlf.eem<TLF.range[1]|data.in$tlf.eem>TLF.range[2]
 #                  |data.in$hlf.eem<HLF.range[1]|data.in$hlf.eem>HLF.range[2],]
...
#for (q in 1:length(list.species)) # species loop
  for (q in c(3,8) ) # species loop ## HAA5 and THM4 only
{
    dat.start -> data.in 
  ...
  if (identical(list.tech, character(0)) ) # check if quantitative model exist
    ...
    if (q == 1) 
      ...
      data.in <- data.frame(data.0$TIME,data.in)
      data.in <- na.omit(data.in)
      data.in <- data.in[!is.infinite(rowSums(data.in[,2:3])),]
      ...
      if (any("NNET" == list.tech)) {
        ...
        for (i in 1:n.rep)    
        {
          ...
          #if (res.q== "NO") {
          #}else{
          pred.mean <- par.resc[1,3]
          pred.sd <- par.resc[2,3]
      ...
      if (q == 3) 
      {out.plot <- data.frame(mean= m.model,sd= sd.model) 
      names(out.plot) <- paste(names(out.plot),"_",list.species[q],sep="")
      }else{ df <- data.frame(mean= m.model,sd= sd.model)
              names(df) <- paste(names(df),"_",list.species[q],sep="")
        out.plot <- cbind(out.plot,df) }
}# end if quantitative model exist
  
} # end loop for diff species

# names(out.plot) <- c(names(out.plot)[1:2],"mean_THM4", "sd_THM4")

d.out <- cbind(Time,out.plot)

# write table out 
write.table(d.out, file=paste(workdir,"/input/VLux_ML-PRED.csv", sep=""),
